        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you sure you want to update this user?", "Update Record", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm = new SqlCommand("UPDATE tbUser SET fullname = @fullname, password=@password, phone=@phone WHERE username LIKE '" + textBoxUserName.Text + "' ", con);
                    cm.Parameters.AddWithValue("@fullname", textBoxFullname.Text);
                    cm.Parameters.AddWithValue("@password", textBoxPassword.Text);
                    cm.Parameters.AddWithValue("@phone", textBoxPassword.Text);
                    con.Open();
                    cm.ExecuteNonQuery();
                    con.Close();
